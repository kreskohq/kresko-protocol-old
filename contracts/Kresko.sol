...
contract Kresko is Ownable {
   ...
   * @notice A mapping of kAsset address to a mapping of user address to
	 ...
	 */
	mapping(
		address => mapping(address => PercentOwnershipInfo)
	 ...
	 */
	mapping(address => uint[]) public collateralLedgers;

	modifier kAssetExists(address kAsset) {
		require(kAssetInfos[kAsset].reserve != address(0), "Kresko: kAsset does not exist");
		_;
	}
	...
	function depositCollateral(address kAsset, uint amount) external kAssetExists(kAsset) {
		require(
			CeloRegistry.getStableToken().transferFrom(msg.sender, kAssetInfos[kAsset].reserve, amount),
			"Kresko: reserve transfer failed"
		);
		uint existingCollateralAmountOwned = getCollateralAmountOwned(kAsset, msg.sender);
	...
	function getTotalCollateralAmount(address kAsset) public kAssetExists(kAsset) view returns (uint) {
		IERC20 stableToken = CeloRegistry.getStableToken();
		return stableToken.balanceOf(kAssetInfos[kAsset].amm) +
			stableToken.balanceOf(kAssetInfos[kAsset].reserve);
	}

	function getCollateralAmountOwned(
		address kAsset,
		address account
	) public kAssetExists(kAsset) view returns (uint) {
		...
		uint[] storage collateralLedger = collateralLedgers[kAsset];
		// Equal to initialPercentageOwned *
		//	(most recent ledger value / ledger value when initialPercentageOwned was valid)
		uint percentageOwned = percentOwnershipInfo.initialPercentageOwned
		...
		return percentageOwned.multiplyDecimalRound(
	...
	function _recordChangeInCollateralLedger(
		address kAsset,
		uint amount,
		uint existingMsgSenderCollateral,
		bool addingCollateral
	) internal {
		// The total collateral amount before adding this new collateral
		uint existingTotalCollateralAmount = getTotalCollateralAmount(kAsset);
		// The total collateral amount after adding this new collateral
		uint newTotalCollateralAmount = addingCollateral ?
		...
		uint totalCollateralChangeFactor = existingTotalCollateralAmount
			.divideDecimalRoundPrecise(newTotalCollateralAmount);

		uint newMsgSenderCollateralAmount = addingCollateral ?
			existingMsgSenderCollateral + amount :
			existingMsgSenderCollateral - amount;
		// High precision decimal.
		// This is the percent of the total collateral that amount corresponds to.
		uint newMsgSenderCollateralPercentage = (newMsgSenderCollateralAmount)
			.divideDecimalRoundPrecise(newTotalCollateralAmount);

		uint[] storage collateralLedger = collateralLedgers[kAsset];

		// Record the new percentage owned
		collateralPercentOwnershipInfos[kAsset][msg.sender].initialPercentageOwned =
			newMsgSenderCollateralPercentage;
		// Record the index of the ledger the above collateral percentage was taken at
		collateralPercentOwnershipInfos[kAsset][msg.sender].ledgerIndex =
			collateralLedger.length;
